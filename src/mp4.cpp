...
static MP4File  *ConstructMP4File ( MP4LogCallback aLogCB = NULL, void* logPtr = NULL )
{
    MP4File* pFile = NULL;
    try {
        pFile = new MP4File();
        pFile->getLogger().setLogCallback(aLogCB);
        pFile->getLogger().setLogPtr(logPtr);
        pFile->getLogger().setVerbosity(MP4_LOG_WARNING);
    }
    catch( std::bad_alloc ) {
...
MP4FileHandle MP4Read( const char* fileName, ShouldParseAtomCallback cb/*=nullptr*/, MP4LogCallback aLogCB , void *logPtr )
{
    if (!fileName)
        return MP4_INVALID_FILE_HANDLE;

    MP4File *pFile = ConstructMP4File(aLogCB, logPtr);
    if (!pFile)
        return MP4_INVALID_FILE_HANDLE;
