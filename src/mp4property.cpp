...
Log& MP4Property::GetLogger()
{
    return m_parentAtom.GetFile().GetLogger();
}
                                          ...
                                          bool dumpImplicits, uint32_t index)
{
    ...
    if (index != 0)
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s[%u] = %u (0x%02x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, index, m_values[index], m_values[index]);
    else
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %u (0x%02x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                                           ...
                                           bool dumpImplicits, uint32_t index)
{
    ...
    if (index != 0)
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s[%u] = %u (0x%04x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, index, m_values[index], m_values[index]);
    else
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %u (0x%04x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, m_values[index], m_values[index]);
}
                                           ...
                                           bool dumpImplicits, uint32_t index)
{
    ...
    if (index != 0)
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s[%u] = %u (0x%06x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, index, m_values[index], m_values[index]);
    else
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %u (0x%06x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, m_values[index], m_values[index]);
}
                                           ...
                                           bool dumpImplicits, uint32_t index)
{
    ...
    if (index != 0)
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s[%u] = %u (0x%08x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, index, m_values[index], m_values[index]);
    else
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %u (0x%08x)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, m_values[index], m_values[index]);
}
                                           ...
                                           bool dumpImplicits, uint32_t index)
{
    ...
    if (index != 0)
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s[%u] = %" PRIu64 " (0x%016" PRIx64 ")",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, index, m_values[index], m_values[index]);
    else
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %" PRIu64 " (0x%016" PRIx64 ")",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, m_values[index], m_values[index]);
}
                               ...
                               bool dumpImplicits, uint32_t index)
{
    ...
    if (index != 0)
        GetLogger().dump(indent, MP4_LOG_VERBOSE1,
                              ...
                              bool dumpImplicits, uint32_t index)
{
    ...
    if (index != 0)
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s[%u] = %f",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, index, m_values[index]);
    else
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %f",
                 m_parentAtom.GetFile().GetFilename().c_str(),
...
void MP4StringProperty::Dump( uint8_t indent, bool dumpImplicits, uint32_t index )
{
    ...
    if( !m_arrayMode ) {
        ...
        if( m_useUnicode )
            GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s%s = %ls",
                     m_parentAtom.GetFile().GetFilename().c_str(),
                     m_name, indexd, (wchar_t*)m_values[index] );
        else
            GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s%s = %s",
                     m_parentAtom.GetFile().GetFilename().c_str(),
                     m_name, indexd, m_values[index] );
    }
    else if( GetLogger().verbosity >= MP4_LOG_VERBOSE2 )
    {
        const uint32_t max = GetCount();

        GetLogger().dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s (size=%u)",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, max );
        ...
        for( uint32_t i = 0; i < max; i++ ) {
            ...
            if( m_useUnicode )
                GetLogger().dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s[%u] = %ls",
                         m_parentAtom.GetFile().GetFilename().c_str(),
                         m_name, i, (wchar_t*)value );
            else
                GetLogger().dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s[%u] = %s",
                         m_parentAtom.GetFile().GetFilename().c_str(),
                         m_name, i, value );
        }
    }
    else {
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": <table entries suppressed>",
                 m_parentAtom.GetFile().GetFilename().c_str() );
    }
}
                            ...
                            bool dumpImplicits, uint32_t index)
{
    ...
    if( size == 0 ) {
        GetLogger().dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s = <%u bytes>",
                 m_parentAtom.GetFile().GetFilename().c_str(),
                 m_name, size );
        return;
    }
    ...
    if( size <= 16 ) {
        ...
        GetLogger().dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s = <%u bytes>%s",
                 m_parentAtom.GetFile().GetFilename().c_str(),
    ...
    if( showall ||
        size < 128 || GetLogger().verbosity >= MP4_LOG_VERBOSE2 )
    {
        adjsize = size;
        supressed = false;
    }
    ...
    GetLogger().dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s = <%u bytes>",
             m_parentAtom.GetFile().GetFilename().c_str(),
             m_name, size );
    GetLogger().hexDump(indent, MP4_LOG_VERBOSE2, value, adjsize, "\"%s\": %s",
                m_parentAtom.GetFile().GetFilename().c_str(),
                m_name);

    if( supressed ) {
        GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": <remaining bytes supressed>",
                 m_parentAtom.GetFile().GetFilename().c_str() );
    }
}
...
void MP4TableProperty::Write(MP4File& file, uint32_t index)
{
    ...
    if (m_pProperties[0]->GetCount() != numEntries) {
        GetLogger().errorf("%s: \"%s\": %s %s \"%s\"table entries %u doesn't match count %u",
        ...
        MP4Property** ppProperty, uint32_t* pIndex)
{
    ...
    GetLogger().verbose1f("\"%s\": matched %s",
                  m_parentAtom.GetFile().GetFilename().c_str(),
                  name);
...
void MP4DescriptorProperty::Read(MP4File& file, uint32_t index)
{
    ...
    if (m_mandatory && m_pDescriptors.Size() == 0) {
        GetLogger().warningf("%s: \"%s\": Mandatory descriptor 0x%02x missing",
                     __FUNCTION__, GetParentAtom().GetFile().GetFilename().c_str(), m_tagsStart);
    } else if (m_onlyOne && m_pDescriptors.Size() > 1) {
        GetLogger().warningf("%s: \"%s\": Descriptor 0x%02x has more than one instance",
                     __FUNCTION__, GetParentAtom().GetFile().GetFilename().c_str(), m_tagsStart);
    }
}
                                 ...
                                 bool dumpImplicits, uint32_t index)
{
    ...
    if (m_name) {
        if (index != 0)
            GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s[%u]",
                     m_parentAtom.GetFile().GetFilename().c_str(),
                     m_name, index);
        else
            GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s",
                     m_parentAtom.GetFile().GetFilename().c_str(),
                     m_name);
        indent++;
    }
...
MP4LanguageCodeProperty::Dump( uint8_t indent, bool dumpImplicits, uint32_t index )
{
    ...
    GetLogger().dump(indent, MP4_LOG_VERBOSE2, "\"%s\": %s = %s (0x%04x)",
             m_parentAtom.GetFile().GetFilename().c_str(),
...
{
    GetLogger().dump(indent, MP4_LOG_VERBOSE1, "\"%s\": %s = %s (0x%02x)",
